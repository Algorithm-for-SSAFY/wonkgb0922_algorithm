...
### 💡접근 방법
> `List<>`의 배열로 간선을 저장했다. 무방향 그래프이기 때문에 양쪽 노드에 각각 저장을 했고, 초기에 `null`일 경우에는 생성자로 초기화를 해주었다.
> 탐색 시에는 `depth`가 5가 될때까지 반복하였고, 5가 되었다면 `true`를 빠르게 리턴하도록 하였다.
> `visited` 배열을 통해 중복 탐색하지 않도록 막았다.

### 🤔 회고
> 자바에서 그래프를 구현하기란 참 귀찮다.
