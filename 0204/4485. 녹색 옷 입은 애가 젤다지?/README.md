...
### 💡접근 방법
> 다익스트라를 이용해서 풀었다. 주어지는 n * n 크기의 2차원 배열을 1차원으로 평탄화하였고, 이 인덱스를 이용해서 다익스트라 알고리즘을 돌렸다.
> 탐색은 1차원의 좌표를 2차원으로 변경해서, 상화좌우를 탐색한 뒤, 실제 좌표는 1차원으로 재변환해서 탐색했다.

### 🤔 회고
> DP도 최적해가 아니었고, DFS도 시간초과가 났다.
> 다익스트라를 처음 공부해서 써봤는데 참 어려웠다.
> 다시 공부해야할 것 같다.
