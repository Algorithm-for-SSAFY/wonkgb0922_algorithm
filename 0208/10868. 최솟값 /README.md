...
### 💡접근 방법
> 세그먼트 트리를 이용해서 최솟값을 구하는 문제이다. 바텁업 방식으로 세그먼트 트리를 구현하였다.

### 🤔 회고
> 이전에 세그먼트 트리를 푼 적은 있었지만, 다시 풀어보니 세그먼트 트리에 대한 이해도를 높일 수 있었다.
> 특히 세그먼트 트리의 범위 결과를 가져올 때 [l, r)이 exclusive해야 하는 이유를 정확히 알았다.
